<x-layouts.portaal title="{{ $persoon->exists ? 'Leiding bewerken' : 'Leiding toevoegen' }} — Scouting">

    <div class="max-w-[600px] mx-auto px-4 pt-10 pb-16">

        <a href="{{ route('scouting.leiding.index') }}" class="text-gedempt text-sm hover:text-accent transition-colors mb-6 inline-flex items-center gap-1.5">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
            Terug naar leiding
        </a>

        <h1 class="text-[1.45rem] font-semibold tracking-tight text-tekst mb-6">
            {{ $persoon->exists ? 'Leiding bewerken' : 'Leiding toevoegen' }}
        </h1>
        ...
        <form method="post"
              action="{{ $persoon->exists ? route('scouting.leiding.bijwerken', $persoon) : route('scouting.leiding.opslaan') }}">
            @csrf
            @if($persoon->exists) @method('PUT') @endif

            <div class="flex flex-col gap-5">
                <div>
                    <label class="block text-sm text-gedempt mb-1.5">Naam *</label>
                    <input type="text" name="naam" value="{{ old('naam', $persoon->naam) }}" required autofocus
            ...
            <div class="flex gap-3 mt-8">
                <button type="submit"
                    ...
                    {{ $persoon->exists ? 'Opslaan' : 'Toevoegen' }}
                </button>
